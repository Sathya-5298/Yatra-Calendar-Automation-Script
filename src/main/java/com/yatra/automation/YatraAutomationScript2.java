...
public class YatraAutomationScript2 {
	...
	public static void main(String[] args) throws InterruptedException 
	{
		...
		driver.get("https://www.yatra.com/"); //Loads the WebPage
		...
		/*
		 * By currentMonthDateElement =
		 * By.xpath(".//div[contains(@class, 'react-datepicker__day')]");
		 * List<WebElement> currentMonthDateList =
		 * currentMonthCalendarElement.findElements(currentMonthDateElement);
		 * 
		 * for( WebElement dates : currentMonthDateList) {
	...
	private static String getMeLowestPrice(WebElement monthElement)
	{
		...
		for( WebElement price : currentMonthPriceList)
		{
			...
			if(!priceValue.isEmpty())
			{
				...
				int priceInt = Integer.parseInt(priceValue); // Convert String to Integer by using Wrapper classes
	...
	public static void compareTwoMonthsPrices(String currentMonthPrice, String nextMonthPrice)
	{
		...
		String currentPrice = currentMonthPrice.substring(currentMonthRsIndex + 2); // from currentMonthPrice
		String nextPrice = nextMonthPrice.substring(nextMonthRsIndex + 2);          // from nextMonthPrice
